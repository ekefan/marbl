...
func TestCreate_InsertsTaskInReceivedState(t *testing.T) {
	...
	resetDB(t, ctx)
	task, err := repo.Create(ctx, domain.TaskType(3), domain.TaskValue(50))
	if err != nil {
...
func TestCreate_TimestampsAreSet(t *testing.T) {
	...
	task, err := repo.Create(ctx, domain.TaskType(0), domain.TaskValue(0))
...
func TestCreate_IDsAreUniqueAndIncreasing(t *testing.T) {
	...
	t1, _ := repo.Create(ctx, domain.TaskType(1), domain.TaskValue(10))
	t2, _ := repo.Create(ctx, domain.TaskType(2), domain.TaskValue(20))
...
func TestGetByID_ReturnsCreatedTask(t *testing.T) {
	...
	created, _ := repo.Create(ctx, domain.TaskType(5), domain.TaskValue(77))
...
func TestUpdateState_ReceivedToProcessing(t *testing.T) {
	...
	task, _ := repo.Create(ctx, domain.TaskType(2), domain.TaskValue(30))

	if err := repo.UpdateState(ctx, task.ID(), domain.StateProcessing); err != nil {
		t.Fatalf("UpdateState() error: %v", err)
	}

	updated, _ := repo.GetByID(ctx, task.ID())
	if updated.State() != domain.StateProcessing {
...
func TestUpdateState_ProcessingToDone(t *testing.T) {
	...
	task, _ := repo.Create(ctx, domain.TaskType(2), domain.TaskValue(30))
	_ = repo.UpdateState(ctx, task.ID(), domain.StateProcessing)
	_ = repo.UpdateState(ctx, task.ID(), domain.StateDone)

	updated, _ := repo.GetByID(ctx, task.ID())
	if updated.State() != domain.StateDone {
...
func TestUpdateState_AdvancesLastUpdateTime(t *testing.T) {
	...
	task, _ := repo.Create(ctx, domain.TaskType(1), domain.TaskValue(10))
...
func TestUpdateState_ErrorOnStateReprocessing(t *testing.T){
	...
	task, _ := repo.Create(ctx, domain.TaskType(2), domain.TaskValue(3))
	_ = repo.UpdateState(ctx, task.ID(), domain.StateProcessing)
	err := repo.UpdateState(ctx, task.ID(), domain.StateProcessing)
...
func TestCountByState_AllStatesPresent(t *testing.T) {
	...
	// create 3 received
	t1, _ := repo.Create(ctx, domain.TaskType(0), domain.TaskValue(10))
	t2, _ := repo.Create(ctx, domain.TaskType(1), domain.TaskValue(20))
	_, _ = repo.Create(ctx, domain.TaskType(2), domain.TaskValue(30))

	// move t1 to processing
	_ = repo.UpdateState(ctx, t1.ID(), domain.StateProcessing)

	// move t2 all the way to done
	_ = repo.UpdateState(ctx, t2.ID(), domain.StateProcessing)
	_ = repo.UpdateState(ctx, t2.ID(), domain.StateDone)
	...
	if counts[domain.StateReceived] != 1 {
		t.Errorf("expected 1 received, got %d", counts[domain.StateReceived])
	}
	if counts[domain.StateProcessing] != 1 {
		t.Errorf("expected 1 processing, got %d", counts[domain.StateProcessing])
	}
	if counts[domain.StateDone] != 1 {
		t.Errorf("expected 1 done, got %d", counts[domain.StateDone])
	}
}
...
func TestCreate_ConcurrentInsertsDontConflict(t *testing.T) {
	...
	for i := range n {
		go func(i int) {
			task, err := repo.Create(ctx, domain.TaskType(i%10), domain.TaskValue(i%100))
			if err != nil {
				errs <- err
				return
			}
