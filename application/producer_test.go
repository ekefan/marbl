...
func newProducer(repo *mockRepo, pub comm_channel.TaskPublisher, maxBacklog int64, rate int) *application.Producer {
	return application.NewProducer(repo, pub, application.ProducerConfig{
...
/// Stopped here, get the mockgen pointing else please
...
func TestProducer_PersistsTaskBeforePublishing(t *testing.T) {
	...
	pub.EXPECT().
		Publish(gomock.Any(), gomock.Any()).
		DoAndReturn(func(ctx context.Context, task *domain.Task) error {
