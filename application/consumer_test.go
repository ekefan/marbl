...
func newConsumer(repo *mockRepo, rateLimit int) *application.Consumer {
	return application.NewConsumer(repo, application.ConsumerConfig{
		RateLimit: rateLimit,
		RateBurst: rateLimit,
	})
}
...
func TestConsumer_UpdatesStateToProcessingThenDone(t *testing.T) {
	...
	task, _ := repo.Create(context.Background(), domain.TaskType(3), domain.TaskValue(5))
...
func TestConsumer_SleepsDurationEqualToValue(t *testing.T) {
	...
	task, _ := repo.Create(context.Background(), domain.TaskType(1), domain.TaskValue(50))
...
func TestConsumer_AggregatesStatsByType(t *testing.T) {
	...
	for _, in := range inputs {
		task, _ := repo.Create(ctx, domain.TaskType(in.typ), domain.TaskValue(in.val))
		if err := consumer.Handler()(ctx, task); err != nil {
			t.Fatalf("Handler() error: %v", err)
		}
	}
	...
	assert.Equal(t, int64(3), counts[domain.StateDone])
}
...
func TestConsumer_RateLimiterThrottlesProcessing(t *testing.T) {
	...
	// create 4 domain with value=0 so sleep is negligible
	taskList := make([]*domain.Task, 4)
	for i := range taskList {
		task, _ := repo.Create(ctx, domain.TaskType(i%10), domain.TaskValue(0))
		taskList[i] = task
	}
	...
	if elapsed < 800*time.Millisecond {
		t.Errorf("rate limiter not throttling: 4 domain done in %v (expected >= 800ms)", elapsed)
	}
}
...
func TestConsumer_StopsOnContextCancelDuringSleep(t *testing.T) {
	...
	task, _ := repo.Create(context.Background(), domain.TaskType(0), domain.TaskValue(99))
...
func TestConsumer_ConcurrentHandlerCallsAreSafe(t *testing.T) {
	...
	if counts[domain.StateDone] != n {
		t.Errorf(
			"expected %d done domain, got %d",
			n,
			counts[domain.StateDone],
		)
	}
...
func TestConsumer_ContinuesProcessingTaskAlreadyInProcessing(t *testing.T) {
	...
	task, err := repo.Create(ctx, domain.TaskType(2), domain.TaskValue(10))
	if err != nil {
		t.Fatalf("Create() error: %v", err)
	}

	// simulate a previously processing domain
	...
	sumByType, err := repo.SumValueByType(ctx)
	assert.NoError(t, err)
	assert.Equal(t, domain.StateDone, updated.State(), "expected recovered task state=done, got %q", updated.State())
