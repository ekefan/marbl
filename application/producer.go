...
type Producer struct {
	...
	publisher comm_channel.TaskPublisher
	cfg       ProducerConfig
	logger    *slog.Logger
}
...
func (p *Producer) produce(ctx context.Context) error {
	...
	currentBacklog := counts[domain.StateReceived] + counts[domain.StateProcessing]
	...
	taskType := domain.TaskType(rand.IntN(10))   // [0, 9]
	taskValue := domain.TaskValue(rand.IntN(100)) // [0, 99]
