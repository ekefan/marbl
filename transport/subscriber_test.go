...
func TestSubscriber_ReceivesSingleTask(t *testing.T) {
	...
	received := make(chan *tasks.Task, 1)
	handler := contracts.TaskHandler(func(ctx context.Context, t *tasks.Task) error {
		received <- t
		return nil
	})
...
func TestSubscriber_ReceivesMultipleTasks(t *testing.T) {
	...
	handler := contracts.TaskHandler(func(ctx context.Context, task *tasks.Task) error {
		mu.Lock()
...
func TestSubscriber_StopsOnContextCancel(t *testing.T) {
	...
	go func() {
		done <- sub.Serve(ctx, func(_ context.Context, _ *tasks.Task) error {
			return nil
		})
	}()
